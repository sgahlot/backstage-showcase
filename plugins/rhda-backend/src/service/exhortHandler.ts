...
export const rhdaSummaryHandler = (req: Request, res: Response) => {
  ...
  clone(repo, gitSourceDir)
    ...
    .catch((error: any) => {
      logger.error(
        `FAILED to clone the repo=[${repo}] to ${gitSourceDir}, with error=${error}`,
      );
    ...
    .then(async () => {
      ...
      childProcess.exec(
        ...
        (error, stdout, stderr) => {
          ...
          res.setHeader(
            'Content-disposition',
            'attachment; filename=RHDA-generated-html-report.html',
          );
          logger.info(
            'html response is generated to the file on server. Going to send the response.',
          );
          // delete the folder after the report.
          deleteFolderIfItExists(gitSourceDir);
